SampleBlog::Admin.controllers :posts do
  ...
  post :create do
    ...
    if @post.save
      ...
      flash[:error] = pat(:create_error, :model => 'post')
      render 'posts/new'
    end
  end
  ...
  put :update, :with => :id do
    ...
    if @post
      if @post.update_attributes(params[:post])
        ...
        flash[:error] = pat(:update_error, :model => 'post')
        render 'posts/edit'
      end
    else
  ...
  delete :destroy_many do
    ...
    posts = Post.find(params[:post_ids].split(','))
    if Post.destroy posts
      flash[:success] = pat(:destroy_many_success, :model => 'Posts', :ids => "#{params[:post_ids]}")
    end
    redirect url(:posts, :index)
  end
end
